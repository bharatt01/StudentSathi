...
import { Home, Search, MapPin, SlidersHorizontal } from 'lucide-react';

const HostelHero = () => {
  return (
    <section className="bg-[#2D2D2D] pt-28 pb-16 relative overflow-hidden">
      <div className="absolute top-20 right-10 w-64 h-64 bg-[#8B6914] opacity-10 rotate-12"></div>
      <div className="absolute bottom-10 left-10 w-48 h-48 bg-[#F4D03F] opacity-5 -rotate-12"></div>
      
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        <div className="flex items-center gap-3 mb-6">
          <div className="bg-[#8B6914] p-2 border-2 border-white rotate-[-3deg]">
            <Home className="w-5 h-5 text-white" />
          </div>
          <span className="text-[#F4D03F] text-xs font-black uppercase tracking-[0.3em]">Browse Hostels</span>
        </div>
        
        <h1 className="text-5xl md:text-6xl font-black text-white uppercase tracking-tight mb-4">
          Find Your <span className="text-[#F4D03F]">Home Away From Home</span>
        </h1>
        <p className="text-[#999] text-lg max-w-2xl mb-10 font-medium">
          Browse 1,200+ verified hostels near your college. Get exclusive deals on rent, food, and security deposit when you book through studentsathi.
        </p>
        ...
        <div className="bg-[#F5F1EB] border-2 border-[#2D2D2D] p-3 shadow-[6px_6px_0px_0px_#000] max-w-3xl">
          <div className="flex gap-2">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder="Search hostel by name or college..."
            ...
            <button className="bg-[#8B6914] hover:bg-[#6B5310] text-white px-6 py-4 font-black uppercase tracking-wide text-sm border-2 border-[#2D2D2D] shadow-[3px_3px_0px_0px_#2D2D2D] hover:shadow-[1px_1px_0px_0px_#2D2D2D] hover:translate-x-[2px] hover:translate-y-[2px] transition-all flex items-center gap-2">
